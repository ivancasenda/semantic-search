...
_METRICS_DESCRIPTOR_VERTEX_AI_PREDICTION = "VertexAIPrediction"
...
def _using_vertex_inference(
    inference_spec_type: model_spec_pb2.InferenceSpecType,
) -> bool:
    return inference_spec_type.WhichOneof("type") == "vertex_ai_prediction_model_spec"
...
def create_model_handler(
    ...
    if _using_vertex_inference(inference_spec_type):
        return _VertexPredictModelHandler(inference_spec_type, options_project_id)
        ...
        if resources.ResourceHint.is_registered("close_to_resources"):
            examples |= "CloseToResources" >> beam.Map(lambda x: x).with_resource_hints(
                close_to_resources=",".join(
                    [
                        RunInferenceImpl._make_close_to_resources(
                            s
...
_Signature = NamedTuple(
    "_Signature", [("name", Text), ("signature_def", _SignatureDef)]
)
...
def _vertex_retry_on_unavailable_and_resource_error_filter(exception: Exception):
    """Retries for google.api_core.exceptions ServiceUnavailable or ResourceExhausted

    Retries if error is unavailable (503) or resource exhausted (429).
    Resource exhausted may happen when qps or bandwidth exceeds quota.

    Args:
      exception: Exception from inference aiplatform.predict execution.

    Returns:
      A boolean of whether retry.
    """

    return isinstance(exception, exceptions.ServiceUnavailable) or isinstance(
        exception, exceptions.ResourceExhausted
    )
...
class _BaseModelHandler(base.ModelHandler, metaclass=abc.ABCMeta):
    ...
    def __init__(self, inference_spec_type: model_spec_pb2.InferenceSpecType):
        operation_type = _get_operation_type(inference_spec_type)
        proximity_descriptor = _METRICS_DESCRIPTOR_CLOUD_AI_PREDICTION
        if _using_in_process_inference(inference_spec_type):
            proximity_descriptor = _METRICS_DESCRIPTOR_IN_PROCESS
        if _using_vertex_inference(inference_spec_type):
            proximity_descriptor = _METRICS_DESCRIPTOR_VERTEX_AI_PREDICTION
...
class _VertexPredictModelHandler(_BaseModelHandler):
    """Performs predictions from a Vertex AI cloud-hosted TensorFlow model.

    Supports both batch and streaming processing modes.
    NOTE: Does not work on DirectRunner for streaming jobs [BEAM-7885].

    In order to request predictions, you must deploy your trained model to Vertex AI
    Model Registry and deploy to an Endpoint in the TensorFlow SavedModel format. See
    [Exporting a SavedModel for prediction]
    ...
    def __init__(
        ...
        super().__init__(inference_spec_type)
        self._vertex_ai_prediction_model_spec = (
            inference_spec_type.vertex_ai_prediction_model_spec
        )
        self._endpoint = None
        project_id = (
            inference_spec_type.vertex_ai_prediction_model_spec.project_id
            or pipeline_options_project_id
        )
        if not project_id:
            raise ValueError(
        ...
        model_name = inference_spec_type.vertex_ai_prediction_model_spec.model_name
        if not model_name:
            raise ValueError("A non-empty model name must be provided.")

        version_name = inference_spec_type.vertex_ai_prediction_model_spec.version_name

        self._model_resource_name = self._get_model_resource_name(
            display_name=model_name
        )
        # If version is not specified, the default version for a model is used.
        if version_name:
            self.model_resouce_name += "@{}".format(version_name)
    ...
    def _make_instances(
        ...
        if self._vertex_ai_prediction_model_spec.use_serialization_config:
            return [
                {"b64": base64.b64encode(se).decode()} for se in serialized_examples
            ]
        else:
    ...
    @staticmethod
    def _get_model_resource_name(display_name: str) -> str:
        """Gets model resource name from Vertex AI Model Registry if exists.

        Args:
          project_id: Google Cloud Platform project id
          display_name: Model name in Vertex AI Model Registry

        Returns:
          Model resource name. Example: "projects/123/locations/us-central1/models/456"
        """
        ...
        if models:
            ...
            raise RuntimeError("Error getting model {}".format(display_name))

        return model_resource_name

    def _get_endpoint_resource_name(self):
        """Gets deployed model's endpoint resource name if exists.
        from `self.model_resouce_name`

        Returns:
          Endpoint resource name. Example: "projects/123/locations/us-central1/endpoints/456".
        """
        endpoint_resource_name = None
        model = aiplatform.Model(self._model_resource_name)
        endpoints = model.gca_resource.deployed_models

        if endpoints:
            endpoint_resource_name = endpoints[0].endpoint
        else:
            raise RuntimeError(
                "Error getting deployed model endpoint from model resouce {}".format(
                    self._model_resource_name
                )
            )
    ...
    def load_model(self):
        """load endpoint for predictions"""
        endpoint_resource_name = self._get_endpoint_resource_name()
        self._endpoint = aiplatform.PrivateEndpoint(endpoint_resource_name)

    # Retry _REMOTE_INFERENCE_NUM_RETRIES times with exponential backoff.
    @retry.with_exponential_backoff(
        initial_delay_secs=1.0,
        num_retries=_REMOTE_INFERENCE_NUM_RETRIES,
        retry_filter=_vertex_retry_on_unavailable_and_resource_error_filter,
    )
    def _execute_request(
        self, instances: List[Mapping[Text, Any]]
    ...
    def _run_inference(
        ...
        self._check_examples(examples)
        instances = self._make_instances(examples, serialized_examples)
        response = self._execute_request(instances)
        return response.predictions
...
class _MultiInferenceModelHandler(_BaseSavedModelHandler):
    ...
    def _post_process(  # pytype: disable=signature-mismatch  # overriding-parameter-type-checks
        ...
        for i, example in enumerate(examples):
            prediction_log = prediction_log_pb2.PredictionLog()
            input_example = (
                prediction_log.multi_inference_log.request.input.example_list.examples.add()
            )
            (
                input_example.ParseFromString
                if isinstance(example, bytes)
...
def _get_signatures(
    model_path: Text, signatures: Sequence[Text], tags: Sequence[Text]
) -> Sequence[_Signature]:
...
def _get_operation_type(inference_spec_type: model_spec_pb2.InferenceSpecType) -> Text:
    if _using_in_process_inference(inference_spec_type):
...
def _get_meta_graph_def(
    saved_model_pb: _SavedModel, tags: Sequence[Text]
) -> _MetaGraphDef:
...
def _get_tags(inference_spec_type: model_spec_pb2.InferenceSpecType) -> Sequence[Text]:
...
class _ModelHandlerWrapper(base.ModelHandler):
    ...
    * Tuple[K, List[ExampleType]] -> Tuple[K, List[PredictType]]
    ...
    def batch_elements_kwargs(self) -> Mapping[str, Any]:
        return self._model_handler.batch_elements_kwargs()
