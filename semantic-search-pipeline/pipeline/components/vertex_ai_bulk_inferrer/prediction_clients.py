...
class AbstractPredictionClient(abc.ABC):
    ...
    def create_model_for_aip_prediction_if_not_exist(
        self, labels: Dict[str, str], ai_platform_serving_args: Dict[str, Any]
    ) -> bool:
...
class CAIPTfxPredictionClient(AbstractPredictionClient):
    ...
    def deploy_model(
        ...
        **kwargs
        ...
        runtime_version = ai_platform_serving_args.get(
        ...
        version_body["runtime_version"] = version_body.get(
            "runtime_version", runtime_version
        )
        version_body["python_version"] = version_body.get(
        ...
        try:
            operation = (
                ...
                .models()
                .versions()
                .create(body=version_body, parent=model_name)
                .execute()
            )
            self._wait_for_operation(operation, "projects.models.versions.create")
            ...
            if e.resp.status == 409:
                ...
                raise RuntimeError(
                    "Creating model version to AI Platform failed: {}".format(e)
                )
        ...
        if set_default:
            ...
            self._client.projects().models().versions().setDefault(
                name="{}/versions/{}".format(model_name, model_version_name)
            ).execute()
    ...
    def create_model_for_aip_prediction_if_not_exist(
        ...
        result = True
        try:
            self._client.projects().models().create(body=body, parent=parent).execute()
        except errors.HttpError as e:
    ...
    def _wait_for_operation(
        self, operation: Dict[str, Any], method_name: str
    ) -> Dict[str, Any]:
        ...
        """
        status_resc = self._client.projects().operations().get(name=operation["name"])
        ...
        if result.get("error"):
            ...
            raise RuntimeError(
                "Failed to execute {}: {}".format(method_name, result["error"])
            )
        return result
    ...
    def delete_model_from_aip_if_exists(
        ...
        if delete_model_endpoint:
            logging.info(
                "Deleting model with from AI Platform: %s", ai_platform_serving_args
            )
            ...
            try:
                ...
                if e.resp.status == 404:
                    ...
                    raise RuntimeError("Deleting model from AI Platform failed.") from e
        else:
            if model_version_name is None:
                raise ValueError(
                    "model_version_name is required if"
                    " delete_model_endpoint is False"
                )
            logging.info(
                "Deleting model version %s from AI Platform: %s",
                model_version_name,
                ai_platform_serving_args,
            )
            ...
            try:
                operation = (
                    ...
                    .models()
                    .versions()
                    .delete(name=version_name)
                    .execute()
                )
                self._wait_for_operation(operation, "projects.models.versions.delete")
            except errors.HttpError as e:
...
class VertexPredictionClient(AbstractPredictionClient):
    ...
    def deploy_model(
        ...
        **kwargs
        ...
        if ai_platform_serving_args.get("project_id"):
            assert "project" not in ai_platform_serving_args, (
                "`project` and "
                "`project_id` should "
                "not be set at the "
            ...
            ai_platform_serving_args["project"] = ai_platform_serving_args["project_id"]
        ...
        unneeded_keys = ["endpoint_name", "project", "regions", "labels"]
        if ai_platform_serving_args["network"]:
            unneeded_keys.append("network")
            unneeded_keys.append("traffic_split")
        else:
        ...
        try:
            ...
            endpoint.deploy(model, **deploy_body)
            endpoint.wait()
        except errors.HttpError as e:
            ...
            if e.resp.status == 409:
                logging.warn(
                    "Model %s already exists at endpoint %s",
                    model_version_name,
                    endpoint_name,
                )
            else:
                raise RuntimeError(
                    "Creating model version to AI Platform failed."
                ) from e
    ...
    def create_model_for_aip_prediction_if_not_exist(
        ...
        endpoint = None
        endpoint_name = ai_platform_serving_args["endpoint_name"]
        endpoint_network = ai_platform_serving_args["network"]

        try:
            self._get_endpoint(ai_platform_serving_args)
        except RuntimeError:
            # Endpoint doesn't exist
            endpoint_labels = {**ai_platform_serving_args.get("labels", {}), **labels}
            try:
                if endpoint_network:
                ...
                raise RuntimeError("Creating endpoint in AI Platform failed.") from e
        else:
    ...
    def delete_model_from_aip_if_exists(
        ...
        if model_version_name is None:
            raise ValueError(
                "model_version_name is required if" " delete_model_endpoint is False"
            )
        logging.info(
            "Deleting model %s from AI Platform: %s",
            model_version_name,
            ai_platform_serving_args,
        )
        ...
        if models:
            ...
            logging.warn(
                "Model %s does not exist at endpoint %s",
                model_version_name,
        ...
        if delete_model_endpoint:
            logging.info(
                "Deleting endpoint with from AI Platform: %s", ai_platform_serving_args
            )
            try:
                endpoint.delete(force=True, sync=True)
                aiplatform.Model(model_to_undeploy.model).delete(sync=True)
            except errors.HttpError as e:
                if e.resp.status == 404:
                    logging.warn(
                    ...
                    raise RuntimeError(
                        "Deleting endpoint from AI Platform failed."
                    ) from e
        else:
            ...
            try:
                ...
                if e.resp.status == 400:
                    logging.warn(
                        "Model %s won't be deleted because it is the "
                        "default version and not the only version in the model",
                        model_version_name,
                    )
                else:
    ...
    def _get_endpoint(
        self, ai_platform_serving_args: Dict[str, Any]
    ) -> aiplatform.Endpoint:
        ...
        if ai_platform_serving_args["network"]:
            endpoints = aiplatform.PrivateEndpoint.list(
...
def get_prediction_client(
    api: Optional[discovery.Resource] = None, enable_vertex: Optional[bool] = False
